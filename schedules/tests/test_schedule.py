...
def test_schedule_copy_week(logged_client, logged_user):
    ...
    result = logged_client.get(
        f"/schedules/v1/get-pair/{target_week.name}/{target_day.day.name}/1/?token={logged_user.username}"
    )
    assert response.status_code == 201
...
def test_schedule_copy_day(logged_client, logged_user):
    ...
    result = logged_client.get(
        f"/schedules/v1/get-pair/{subject_monday.week.name}/{subject_friday.day.name}/1/?token={logged_user.username}"
    )
...
def test_schedule_copy_pair(logged_client, logged_user):
    ...
    result = logged_client.get(
        f"/schedules/v1/get-pair/{subject_monday.week.name}/{subject_friday.day.name}/{target_pair}/?token={logged_user.username}"
    )
...
def test_get_schedule_one_field(logged_user, logged_client):
    ...
    response = logged_client.get(
        "/schedules/v1/schedule/get_one_field/?token={}&select_field=teacher".format(
            logged_user.username
        )
    )
...
def test_get_pair_schedule(logged_user, logged_client):
    ...
    response = logged_client.get(
        "/schedules/v1/get-pair/{week}/{day}/{number}/?token={token}".format(
...
def test_get_type_pair(not_logged_client):
    ...
    response = not_logged_client.get("/schedules/v1/type-pair/")
