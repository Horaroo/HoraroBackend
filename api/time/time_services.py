...
def _pair_model_to_dict(pair: Schedule) -> dict:
    fields = ['subject', 'teacher', 'audience',
              'week', 'group', 'type_pair', 'day']
...
class TimeServices:
    ...
    def get_pair_status(self, time: Time = None) -> PairStatus:
        ...
        for pair_num, time_range in enumerate(pairs):
            if time in time_range:
                return PairStatus(status='pair', pair_num=pair_num, time_range=schedule[pair_num])

        breaks = [TimeRange(start=pairs[i - 1].end, end=pairs[i].start)
                  for i in range(1, len(pairs))]
        for break_num, _break in enumerate(breaks):
            if time in _break:
                return PairStatus(status='break', pair_num=break_num + 1, time_range=schedule[break_num + 1])
    ...
    def get_current_pair(self, pair_qs, time: Time = None) -> PairStatus:
        ...
        if pair_status.status == 'nothing':
            return PairStatus(status='nothing',
                              pair_num=None)
        if pair_status.pair_num + 1 < pair_qs[0].number_pair or \
                pair_status.pair_num + 1 == pair_qs[0].number_pair and pair_status.status == 'break':
            return PairStatus(status='early',
        ...
        for pair in pair_qs:
            if pair.number_pair == pair_status.pair_num + 1:
                if pair_status.status == 'break':
                    return PairStatus(status='break',
