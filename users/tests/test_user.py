...
def test_user_self_delete(logged_client, logged_user):
    ...
    response = logged_client.delete(
        path="/schedules/v1/auth/detail/me/", data={"current_password": "test"}
    )
...
def test_user_self_get(logged_client, logged_user):
    response = logged_client.get(path="/schedules/v1/auth/detail/me/")
...
def test_create_auth_token_and_destroy(logged_client, logged_user):
    ...
    response = logged_client.post(
        path=f"/schedules/v1/auth/token/login/",
    ...
    response = logged_client.post("/schedules/v1/auth/token/logout/")
